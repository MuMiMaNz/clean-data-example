...
SELECT  e.cid, tb4.hospcode, tb4.pid, tb4.seq, tb4.date_serv, tb4.sbp, tb4.dbp, tb4.diagcode,
    --     CASE
    --         WHEN tb4.diagcode like "I1%" THEN "HT"
    --         WHEN tb4.diagcode like "E1%" THEN "DM"
    --         WHEN tb4.diagcode like "I48%" THEN "AF"
    --         WHEN tb4.diagcode like "E78%" THEN "DLD"
    --         WHEN tb4.diagcode like "I2%" THEN "MI"
    --      ELSE NULL
    --  END AS diagcode,
...
FROM
    ...
    SELECT  tb5.hospcode, tb5.pid, x.seq, tb5.diagcode, tb5.date_serv, x.sbp, x.dbp
    FROM
     (
     SELECT  c.hospcode, c.pid, regexp_replace(trim(upper(c.chronic)),"\\.","") as diagcode, c.date_diag as date_serv
     FROM    hdc.chronic c
     WHERE   diagcode like "I1%"
     )tb5
